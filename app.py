...
app = Flask(__name__)
app.secret_key = "your_secret_key"  # i have to make it secure in producn
DATABASE = "instance/app.db"


def init_db():
    # To make sure there is  instance folder exists
    os.makedirs(os.path.dirname(DATABASE), exist_ok=True)
    conn = sqlite3.connect(DATABASE)
    c = conn.cursor()

    # now Creating tables if they do not exist
    c.execute("""
    ...
    # now Inserting  default admin user if not exists
...
#for adding parking lots by the admin
...
#to allow the user to delete  a parking lot
...
def release(reservation_id):
    ...
    #now i have to  Calculate the duration hours
